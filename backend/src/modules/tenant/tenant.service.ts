...
export class TenantService {
  ...
  async create(dto: CreateTenantDto): Promise<Tenant> {
    const [existing] = await this.db.select().from(tenants).where(eq(tenants.document, dto.document)).limit(1);
    ...
    const [created] = await this.db.select().from(tenants).where(eq(tenants.document, dto.document)).limit(1);
  ...
  async update(id: string, dto: UpdateTenantDto): Promise<Tenant> {
    ...
    if (dto.document) {
      const [existing] = await this.db.select().from(tenants).where(and(eq(tenants.document, dto.document), ne(tenants.id, id))).limit(1);
    ...
    await this.db.update(tenants).set(dto).where(eq(tenants.id, id));
