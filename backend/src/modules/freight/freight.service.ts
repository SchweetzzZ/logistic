...
export class FreightService {
  ...
  async simulateFreight(
    ...
    const volumetricWeightKg =
      Math.round(((length * width * height) / 6000) * 1000) / 1000;
    ...
    if (dto.carrierId && carriers.length === 0) {
      throw new NotFoundException(
        'Transportadora informada não foi encontrada ou está inativa nesta empresa',
      );
    }
    if (carriers.length === 0) {
      throw new NotFoundException(
        'Nenhuma transportadora ativa encontrada para realizar a cotação nesta empresa',
      );
    }
    ...
    const quotes: FreightQuote[] = carriers
      .map((carrier) => {
        ...
        const shippingSubtotal =
          Math.round((basePrice + weightCost) * distanceMultiplier * 100) / 100;
        const totalPrice =
          Math.round((shippingSubtotal + insuranceCost) * 100) / 100;
